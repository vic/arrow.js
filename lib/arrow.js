...
Arrow.next = function () {
  return slicer(arguments).reduce(function (previous, current) {
    return Arrow(function next () {
      var args = slicer(arguments, 0, -1)
      var cont = slicer(arguments, -1)[0]
      previous.apply(null, args.concat(function () {
...
Arrow.first = function (arrow) {
  return Arrow(function first (first) {
    ...
    var cont = slicer(arguments, -1)[0]
    arrow.apply(null, [].concat(first, function () { 
...
Arrow.array = function (n) {
  return Arrow(function array () {
    var args = n ? slicer(arguments, 0, n) : slicer(arguments, 0, -1)
    var rest = n ? slicer(arguments, n, -1) : []
    var cont = slicer(arguments, -1)[0]
...
Arrow.pureN = function (pure, i, j) {
  return Arrow.next(Arrow.array(i), Arrow.first(Arrow.pure(pure)), Arrow.drop(j)) 
}

Arrow.cpsN = function (cps, i, j) {
  return Arrow.next(Arrow.array(i), Arrow.first(Arrow(cps)), Arrow.drop(j))
}
